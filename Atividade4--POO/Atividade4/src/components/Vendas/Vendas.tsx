...
const Vendas: React.FC = () => {
  ...
  // Atualiza o nome do cliente ao consultar pelo ID
  const handleClienteIdChange = async (id: string) => {
    setClienteId(id);
    setErro("");
    setClienteNome("");
    ...
    try {
      ...
      if (response.status === 200) {
        setClienteNome(response.data.nome); // Define o nome do cliente
      } else {
      ...
      if (error.response) {
        setErro(`Erro: ${error.response.data.message || "Cliente não encontrado"}`);
      } else {
  ...
  // Caso o nome do cliente seja alterado
  const handleClienteNomeChange = async (nome: string) => {
    setClienteId(""); // Limpa o ID
    setErro("");
    setClienteNome(nome);
    ...
    try {
      const response = await axios.get(`http://localhost:32832/clientes?nome=${nome}`);
      if (response.status === 200 && response.data.length > 0) {
        setClienteNome(response.data[0].nome); // Define o nome do cliente
      } else {
        setErro("Cliente não encontrado.");
      }
    } catch (error) {
  ...
  return (
    <Box className="vendas-container">
      ...
      <Box className="vendas-input">
        ...
        />
        {erro ? (
          <Typography className="cliente-nome" variant="body1" color="error">
            {erro}
          </Typography>
        ) : (
          <Typography className="cliente-nome" variant="body1" color={clienteNome ? "textPrimary" : "textSecondary"}>
